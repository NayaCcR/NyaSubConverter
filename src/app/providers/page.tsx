...
import { AlertTriangle, Check, CloudDownload, Download, Pencil, Plus, RefreshCw, ServerCog, Star, Trash2 } from "lucide-react";
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 px-4 py-7 sm:px-6 lg:py-9">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2"><span className="text-xs text-muted-foreground">{notice}</span>{isDevelopment && <Button variant="outline" type="button" onClick={exportPublicList} className={buttonSecondary}><Download className="h-4 w-4" />导出公共列表</Button>}<Button variant="outline" type="button" onClick={refreshPublicList} disabled={refreshing} className={buttonSecondary}><CloudDownload className={cn("h-4 w-4", refreshing && "animate-spin")} />更新公共列表</Button></div>
      </div>
          ...
          {providers.map((provider) => (
            <div key={provider.id} className={cn("grid gap-4 px-4 py-4 md:grid-cols-[minmax(180px,1fr)_150px_120px_110px_160px] md:items-center md:px-5", !provider.enabled && "opacity-55")}>
              ...
              <div className="flex items-center justify-end gap-1">
                <Button variant="ghost" size="icon" type="button" title="测试连接" onClick={() => testConnection(provider)} disabled={provider.status === "checking"} className={iconButton}><RefreshCw className={cn("h-4 w-4", provider.status === "checking" && "animate-spin")} /></Button>
                {!provider.isDefault && <Button variant="ghost" size="icon" type="button" title="设为默认" onClick={() => setProviders((items) => items.map((item) => ({ ...item, isDefault: item.id === provider.id })))} className={iconButton}><Star className="h-4 w-4" /></Button>}
                <Button variant="ghost" size="icon" type="button" title="编辑" onClick={() => setEditing(provider)} className={iconButton}><Pencil className="h-4 w-4" /></Button>
                <Button variant="ghost" size="icon" type="button" title="删除" onClick={() => { if (window.confirm(`确定删除“${provider.name}”吗？`)) setProviders((items) => items.filter((item) => item.id !== provider.id)); }} className={cn(iconButton, "hover:text-destructive")}><Trash2 className="h-4 w-4" /></Button>
              </div>
            </div>
          ))}
        </div>
