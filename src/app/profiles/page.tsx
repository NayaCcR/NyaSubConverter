...
import { Check, Copy, FileSliders, Pencil, Plus, Trash2, X } from "lucide-react";
      ...
      {profiles.length ? (
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {profiles.map((profile) => (
            <article key={profile.id} className="flex min-h-52 flex-col rounded-lg border border-border bg-card p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                ...
                <div className="flex gap-1">
                  <button title="复制" type="button" onClick={() => setProfiles((items) => [...items, { ...profile, id: uid("profile"), name: `${profile.name} 副本`, updatedAt: new Date().toISOString() }])} className={iconButton}><Copy className="h-4 w-4" /></button>
                  <button title="编辑" type="button" onClick={() => setEditing(profile)} className={iconButton}><Pencil className="h-4 w-4" /></button>
                  <button title="删除" type="button" onClick={() => { if (window.confirm(`确定删除“${profile.name}”吗？`)) setProfiles((items) => items.filter((item) => item.id !== profile.id)); }} className={cn(iconButton, "hover:text-destructive")}><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
...
function ProfileDialog({ profile, onClose, onSave }: { profile: Profile | null; onClose: () => void; onSave: (profile: Profile) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/45 p-4 backdrop-blur-sm">
      <form onSubmit={(event) => { event.preventDefault(); if (form.name.trim()) onSave({ ...form, name: form.name.trim(), updatedAt: new Date().toISOString() }); }} className="mx-auto my-4 w-full max-w-4xl animate-scale-in rounded-lg border border-border bg-card p-5 shadow-2xl sm:p-6">
        <div className="flex items-center justify-between"><div><h2 className="text-base font-semibold">{profile ? "编辑 Profile" : "新建 Profile"}</h2><p className="mt-1 text-xs text-muted-foreground">完整保存首页支持的全部 SubConverter 参数</p></div><button type="button" onClick={onClose} className={iconButton}><X className="h-4 w-4" /></button></div>
        ...
        <div className="mt-6 flex justify-end gap-2 border-t border-border pt-5"><button type="button" onClick={onClose} className={buttonSecondary}>取消</button><button type="submit" disabled={!form.name.trim()} className={buttonPrimary}><Check className="h-4 w-4" />保存</button></div>
      </form>
    </div>
  );
}
